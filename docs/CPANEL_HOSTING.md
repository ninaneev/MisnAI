# Hosting Taskoona on cPanel

Taskoona is a static Vite app. It does not need Node, a backend server, Vercel, or paid hosting for the first public version.
...
Upload the contents of `dist/` to the document root for `taskoona.flowity.ai`.

## DNS and Subdomain

Use the existing Flowity hosting setup:

1. Create the `taskoona.flowity.ai` subdomain in cPanel.
...
Do not add hosted AI, sync, auth, billing, or server processes to this deployment. Those belong in the private `taskoona-cloud` repo later.
