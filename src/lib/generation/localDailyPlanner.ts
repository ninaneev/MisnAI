...
export function generateLocalDailySteps({
  profile,
  pendingTasks,
}: LocalDailyPlanInput): Record<string, string[]> {
  ...
  const templateMove = template?.dailyMoves[0] ?? null

  return {
    'morning-review': [
      task1
        ? `Today's build priority: ${task1.label} - ${truncate(task1.description, 8)}`
        : templateMove ?? 'Open your strategy and identify the single most important task today.',
      topGoal
    ...
    'deep-work-1': task1
      ...
      : [
          templateMove ?? 'Open your strategy and pick the single highest-leverage task.',
    ...
    outreach: [
      task2
        ...
        : template?.id === 'zero-to-first-customer'
          ? 'Message 3 specific people who plausibly feel the problem you want to solve.'
    ...
    'midday-check': [
      ...
      task2 ? `Afternoon target: ${task2.label}` : 'Choose your afternoon focus before reopening any work.',
    ],
    ...
    'skill-sharpening': [
      task3
        ...
        : template?.focusAreas[0]
          ? `Choose one narrow topic tied to ${template.focusAreas[0]}.`
    ...
    'evening-review': [
      task1 ? `Status on "${task1.label}": what actually happened?` : 'Name the one thing that genuinely moved today.',
      "Assess whether today matched this morning's intention.",
      task1 ? `Tomorrow's first move: what exactly needs to happen on "${task1.label}"?` : "Name tomorrow's first move while it's still obvious.",
    ],
    ...
    'strategy-time': [
      task3
        ...
        : template
          ? `Template focus: ${template.title} - ${template.decisionPrompts[0]}`
          : topBusinessGoal
